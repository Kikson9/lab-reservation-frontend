...
function getStatusBadge(available, total) {
  if (available === 0) {
    return <span className="px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-600">Full</span>;
  } else if (available / total < 0.2) {
    return <span className="px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-600">Almost Full</span>;
  } else {
    return <span className="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-600">Open</span>;
  }
}
            ...
            <div>
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mb-6">
                <div
                  ...
                  style={{width: `${((lab.totalSeats - lab.availableSeats) / lab.totalSeats) * 100}%`}}
                ></div>
              </div>
            </div>
